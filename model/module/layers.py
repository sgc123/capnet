...
def _update_routing(votes, biases, num_routing):
    ...
    #   修改过 由biases[x,y,z] 修改为 biases[x,y,z，1，1] 
    biases=biases.unsqueeze(3)
    biases=biases.unsqueeze(4)
...
class ConvSlimCapsule3D(nn.Module):
    ...
    def __init__(
        ...
        self.biases = nn.Parameter(torch.nn.init.constant_(torch.empty(output_dim, output_atoms, 1), 0.1))
