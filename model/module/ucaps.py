...
class Net(pl.LightningModule):
    ...
    def forward(self, x):
        ...
        x = x.unsqueeze(dim=1)
        conv_cap_1_1 = self.primary_caps(x)  #64 32 32  16 4

        x = self.encoder_conv_caps[0](conv_cap_1_1) #  降
        conv_cap_2_1 = self.encoder_conv_caps[1](x)  

        x = self.encoder_conv_caps[2](conv_cap_2_1)   # 16 16 8 8 16
...
def main():
    X_tensor = torch.ones((4,1,32, 64, 64))
    Y_tensor = torch.zeros((4,1,32, 64, 64))
